...
def get_context(context=None):
   ...
   assigned_tasks_tomorrow = frappe.get_all(
       "Assigned Tasks",
       filters={"date": date},
   ...
   def get_not_available(slot_time):
       return [t.technician for t in assigned_tasks_tomorrow if t.stime <= slot_time < t.etime]
   ...
   for tech in technicians:
       ...
       while i < len(BASE_TIME_SLOTS):
           ...
           if task_in_slot:
               ...
               html_content += f"""
               <div style="width: {task_in_slot['duration_in_hours'] * 2 * slot_width_percent}%; background-color: #ef4444; border-right: 1px solid #fff; border-radius: 4px; overflow: hidden; cursor: grab; user-select: none;" class="px-1 py-2 text-white text-center drag" data-type="type2" draggable="true" id="task-{task_in_slot['issue_code']}" data-duration="{task_in_slot['duration_in_hours']}" title="{task_in_slot['issue_code']}">
               ...
               """
               html_content += """
               <div class="modal hide" id="taskModaltask-{issue_code}" tabindex="-1" role="dialog"
                   aria-labelledby="taskModalLabel{issue_code}" aria-hidden="true">
                   <div class="modal-dialog" role="document" style="max-width: 80%; margin: 1.75rem auto">
                       <div class="modal-content">
                           <div class="modal-header">
                               <h5 class="modal-title" id="taskModalLabel{issue_code}">{issue_code}</h5>
                               <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                   <span aria-hidden="true">&times;</span>
                               </button>
                           </div>
                           <div class="modal-body">
                               <form id="custom2-form-{issue_code}" class="custom-form" method="POST">
                                   <label for="code">Maintenance Visit Code:</label>
                                   <input class="form-control code clickable-code" type="text" name="code" value="{issue_code}" required
                                       readonly style="cursor: pointer;" onclick="window.open('/app/maintenance-visit/{issue_code}', '_blank')"><br><br>
           ...
           html_content += (f'<div style="width: {slot_width_percent}%; border-right: 1px solid #bae6fd; '
                             ...
                             f'data-tech="{tech.email}" data-na="{not_available}" '
