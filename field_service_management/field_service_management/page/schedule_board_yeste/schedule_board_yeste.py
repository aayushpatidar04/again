...
def get_context(context=None):
   ...
   # Only show technicians explicitly enabled on their Employee record.
   technician_emails = [tech.email for tech in technicians]
   if technician_emails:
       displayable_emails = set(frappe.get_all(
           "Employee",
           filters={
               "user_id": ["in", technician_emails],
               "custom_display_on_scheduling_board": 1,
           },
           pluck="user_id",
       ))
   ...
   for tech in technicians:
       ...
       if(leaves):
           for leave in leaves:
               if (leave.half_day == 1):
                   ...
                   count = 12
                   afternoon = 0
                   html_content += f'<div style="width: 1200px; border-right: 1px solid #fff; color: white; background-color: #7c3aed; border-radius: 4px; overflow: hidden;" data-tech="{tech.email}" class="px-1">{leave.description if leave.description else 'Leave'}</div>'
       else:
           count = 0
           afternoon = 0
       total_hours = 0
       ...
       for slot in time_slots:
           if count == -0.5:
               ...
               html_content += f'<div style="width: 50px; border-right: 1px solid #bae6fd; background-color: #e0f2fe; border: 2px dashed #ccc; min-height: 40px;" data-time="{ttt}" data-tech="{tech.email}" class="px-1">-</div>'
           ...
           if slot['label'] == '01:00 PM' and afternoon == 1:
               count += 8
               html_content += f'<div style="width: 800px; border-right: 1px solid #fff; color: white; background-color: #7c3aed; border-radius: 4px; overflow: hidden;" data-tech="{tech.email}" class="px-1">Leave</div>'
           if slot['label'] == '12:00 PM':
               if(count >= 1):
                   count -=1
               elif(count == 0.5):
                   html_content += f'<div style="width: 50px; border-right: 1px solid #fff; color: white; background-color: #f59e0b; border-radius: 4px; overflow: hidden;" data-time="{slot["time"]}" data-tech="{tech.email}" class="px-1">Lunch Time</div>'
                   count -= 0.5
               else:
                   html_content += f'<div style="width: 100px; border-right: 1px solid #fff; color: white; background-color: #f59e0b; border-radius: 4px; overflow: hidden;" data-time="{slot["time"]}" data-tech="{tech.email}" class="px-1">Lunch Time</div>'
           else:
               not_available = []
               ts = frappe.get_all(
                   "Assigned Tasks",
               ...
               if task_in_slot:
                   ...
                   html_content += f"""
                   <div style="width: {task_in_slot['duration_in_hours'] * 100}px; background-color: #ef4444; border-right: 1px solid #fff; border-radius: 4px; overflow: hidden; padding: 10px; cursor: grab; user-select: none;" class="px-1 py-2 text-white text-center drag" data-type="type2" draggable="true" id="task-{task_in_slot['issue_code']}" data-duration="{task_in_slot['duration_in_hours']}">
...
+                                            readonly style="cursor: pointer;" onclick="window.open('/app/maintenance-visit/{task_in_slot['issue_code']}', '_blank')"><br><br>
                   ...
                   if count == 0:
                       html_content += f'<div style="width: 100px; border-right: 1px solid #bae6fd; background-color: #e0f2fe;" data-time="{slot["time"]}" data-tech="{tech.email}" data-na="{slot["not_available"]}" class="px-1">-</div>'
                    #    html_content += f'<div style="width: 100px; border-right: 1px solid #000; background-color: #78D6FF;" data-time="{slot["time"]}" data-tech="{tech.email}" data-na="{slot["not_available"]}" class="px-1">-</div>'
                       ...
                       html_content += f'<div style="width: 50px; border-right: 1px solid #bae6fd; background-color: #e0f2fe;" data-time="{slot["time"]}" data-tech="{tech.email}" data-na="{slot["not_available"]}" class="px-1">-</div>'
